...
import {
  ndnDice
} from "../commands/utils/dice.mjs"
import {
  dominoeffect
} from "../commands/utils/domino.mjs"
import {
  createEmbed,
...
export default async (message) => {
  ...
  if (message.content.match(/(^(こころ|ココロ|心)…*$|ココロ…|ココロー！)/)) {
    if (Math.floor(Math.random() * 100) < 1) { //0-99 1%で大当たり　ココロー！
      ...
      const toruchan = ['1264756212994674739', '1265162812758687754', '1265163072016879636', '1265163139637317673', '1265163377538236476'];
      ...
      try {
        ...
        if (images.length > 1) {
          for (let i = 1; i < images.length; i++) {
            const imageEmbed = createEmbed(fullMatch, null, null, {
              displayName: null,
              displayAvatarURL: () => null
            }, images[i], fetchedMessage.createdAt, '#0099ff', null);
          ...
          if (refMessage) {
            ...
            if (refImages.length > 1) {
              for (let i = 1; i < refImages.length; i++) {
                const refImageEmbed = createEmbed(refMatch, null, null, {
                  displayName: null,
                  displayAvatarURL: () => null
                }, refImages[i], refMessage.createdAt, '#B78CFE', null);
