...
export default {
    ...
    async execute(oldMember, newMember) {
        ...
        const boosterRoleId = config.chatBonus.booster_coin.roles[newMember.guild.id];
        ...
        if (!oldHasRole && newHasRole) {
            console.log(`[BOOSTER] ${newMember.user.tag} started boosting in ${newMember.guild.name}.`);
        ...
        // ブースト終了
            ...
            console.log(`[BOOSTER] ${newMember.user.tag} stopped boosting in ${newMember.guild.name}.`);
            const { error } = await supabase
                .from('booster_status')
                .delete()
