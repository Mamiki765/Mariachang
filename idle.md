...
##### 7.3. インフィニティアップグレード(未実装)
IPを消費して購入できる、インフィニティ後のスキルです。
Tierを制覇後、次のTierが解禁される。
###### **Tier 1**(13から未実装)
| ID | アップグレード名 | コスト | 効果 |
| :--- | :--- | :--- | :--- |
| `IU11` | **ゴーストチップ Lv1** | 0 IP | リセット(プレステージ,インフィニティ)時に無償で得る5000チップの範囲で、工場を自動強化する。 |
| `IU12` | **自動調理器** | 1 IP | TPお任せアップグレードの解禁 |
| `IU13` | **肉干しレンガ** | 1 IP | 精肉工場の指数に `+0.05` のボーナス。 |
| `IU14` | **業務用品の購入** | 1 IP | チップ消費量x0.9 |
| `IU21` | **パインパワー** | 3 IP | PP獲得量+10%(PP-7のSPも増える) |

*  **ゴーストチップ:** 500000チップでレベルアップし5000チップを更に得ることができる。費用は2倍、3倍と増えていき（最大10倍=500万）、最大Lvは200(100万チップ)

##### 7.4. インフィニティチャレンジ(IC)(未実装)
自身に縛りを課してInfinityを目指すエンドコンテンツ。
条件を満たしたら取れるか、呪いを受けてインフィニティをリセットする形で開始。攻略順は自由
例）
| 試練 | 名前 | 縛り内容 | 報酬 |
| IC1 | 祝福無き道 | #7がLv0でインフィニティ | #7の計算式にさらにエタニティチップを加算 |
| IC2 | 崩壊寸前の世界 | ゲーム内24時間でクリアする | SPスキル #2効果 ^ 0.5 が上位乗算施設に乗る(2乗→3.5乗) |
| IC3 | アドブロック | アセンションが弱体化 | アセンションが強化 |
| IC4 | 平等な才 | 呪い(精肉施設の基本Lv(Mee6)が30で固定される) | IPx2 |
| IC5 | 貧者の嫉妬 | 1000万チップ以下でインフィニティ | チップx1.1 |
…

* **報酬:** それぞれの試練にちなんだ報酬が得られる他、IPが(1 + ICクリア数)倍になる。
