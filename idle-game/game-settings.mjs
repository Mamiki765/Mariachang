...
export default {
  ...
  infinityMessages: {
    // ブレイクインフィニティ (1.8e308超え)
    breakInfinity: (data) => `# ●${data.population} Break Infinity
## ――ニョワミヤはどこまで増えるのだろう。
...
**${data.ip} IP** と **${data.infinities} ∞** を手に入れた。`,
    ...
    // 初回インフィニティ
    firstInfinity: (data) => `# ●1.79e+308 Infinity
## ――あなたは果てにたどり着いた。
終わりは意外とあっけないものだった。
...
ピザ生産ジェネレーターが解禁された。`,
    ...
    // 通常インフィニティ (2回目以降)
    normalInfinity: (data) => `# ●1.79e+308 Infinity
## ――あなたは果てにたどり着いた。
終わりは意外とあっけないものだった。
ピザを求めてどこからか増え続けたニョワミヤ達はついに宇宙に存在する全ての分子よりも多く集まり、
それは一塊に集まると、凄まじい光を放ち膨張し……そして新たな星が誕生した。
...
**${data.ip} IP** と **${data.infinities} ∞** を手に入れた。`
  ...
  eternity: {
    ...
    */
    ...
    messages: {
      // 初回エタニティ用
      firstEnding: (data) => `# ●1.79e+308 IP  Eternity
## ――あなたは。
## この世界の終わりに辿り着いた。
...
**${data.ep} EP** と **${data.sigma} Σ** を手に入れた。
所持しているニョボチップと今までの全てを失った。
エタニティストーンが解禁された。`,

      // 2回目以降用
      normalEnding: (data) => `# ●Eternity
## ――あなたは再び世界の終わりに辿り着いた。
　そして、再びピザ窯は動き出した。

- **今回のエタニティまでのゲーム内時間:** ${data.gameTime}
- **今回のエタニティまでの現実時間:** ${data.realTime}
- **エタニティ最短記録 (現実時間):** ${data.bestRealTime}
...
-# そしてなぜか次の工場は強化されました。`,
      ...
      // TPのみプレステージ
      tpOnly: (data) => `●TPプレステージ
# そうだ、サイドメニュー作ろう。
あなた達は${data.population}匹のニョワミヤ達と一緒にサイドメニューを作ることにしました。
