...
| `IU91` | **無(未実装)**             | 1e40 IP     | 最終強化要素の解禁（仮）(未実装)                                                             |
...
IU91を購入することで解禁される要素。
ギャラクシーを購入することでグラビティが産出され、爆発的にジェネレーターが強化される。

Revolution Idleの丸パクリになっちゃうけど…（小声）

#### 9.エタニティ（Σ）

ゲーム全体。エタニティ中のチップ消費量や経過時間は、インフィニティでもリセットされることはない。
1.79e+308IP でエンディングとなり、EP（未実装）とΣ（未実装）が追加される。

| Σ回数 | 8工場倍率 | チップ収量 | Ascen-Power | IP Gain | GP Gain | グラビティ | 特殊効果                                                                |
| :---: | :-------: | :--------: | :---------: | :-----: | :-----: | :--------: | :---------------------------------------------------------------------- |
| **1** |  `x2.00`  |  `x2.00`   |   `x1.10`   | `x2.00` | `x2.14` |   `x10`    | チップ収量が最大値を維持するようになる (初回はインフィニティ前の倍率x2) |
| **2** |  `x3.00`  |  `x3.00`   |   `x1.20`   | `x3.00` | `x3.36` |   `x100`   | EP強化要素の解禁                                                        |
| **3** |  `x4.00`  |  `x4.00`   |   `x1.30`   | `x5.00` | `x4.59` |  `x1,000`  | 最短インフィニティ記録とIC9クリア時間が1msとして扱われるようになる      |
| **4** |  `x5.00`  |  `x5.00`   |   `x1.40`   | `x6.00` | `x5.87` | `x10,000`  | 工場を購入してもチップが減らなくなる                                    |
| **5** |  `x6.00`  |  `x6.00`   |   `x1.50`   | `x8.00` | `x7.18` | `x100,000` | **ブレイクエタニティ**: `1.79e308 IP` の上限が撤廃され、EPがIPに依存    |

- **※ギャラクシー:** 将来的に実装予定のジェネレーター強化要素。
- **ソフトキャップ:** 一定のΣ回数を超えると、各ボーナスの伸び率は緩やかになる可能性があります。

### 10. ランキングスコア (Rank Score) について
