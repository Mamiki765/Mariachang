...
| `IU54` | **先行投資**                 | 10000 IP   | ゴーストチップ強化、LvCap200 -> 999 、インフィニット時別個のチップでアセンションも行う（人口無視）      |
| `IU61` | **大型発電機**               | 1000000 IP | G1を10倍強化                                                                                            |
| `IU62` | **並行宇宙観測装置**         | 2000000 IP | 獲得∞がlog10(チップ枚数 + 1) + 1倍になる(小数点以下切り捨て)                                            |
| `IU63` | **空間歪曲マシン**           | 1000000 IP | ∞に応じてG3強化（max(log10(∞+1)x 0.80,1) 未実装）                                                                 |
| `IU64` | **？？？？（未定義）**       | 3000000 IP | ∞に応じてチップ獲得量増加or値段割引（未実装、効果未定義）                                               |
| `IU71` | **無限循環装置**             | 1e11 IP    | ジェネレーター3号機の出力が(2号機の購入数+1)倍(未実装)                                                  |
| `IU72` | **エントロピー縮小機関**     | 1e11 IP    | ジェネレーター4号機の出力が(3号機の購入数+1)倍 (未実装)                                                 |
| `IU73` | **不思議な望遠鏡（未実装）** | 2e11 IP    | 取得後最短の∞時間を計測開始、最短infinity（現実時間準拠）の1/50~30(現実時間準拠、考え中)の速度で∞を獲得 |
| `IU74` | **歪んだ歯車**               | 1e12 IP    | ジェネレーター指数が0.75 -> 0.9(未実装)                                                                 |
