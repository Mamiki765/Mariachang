...
ゲーム全体の最終目標。
1.79e+308IP でエンディングとなり、EPとΣが追加される。
...
| 6   | ギャラクシー・グラビティアプグレの自動購入                  |                                                         |
| 10  | エタニティブレイク                                          | IPの上限解除                                            |
| 12  | インフィニティチャレンジのスキップ                          | IC9のクリア時間は最短インフィニティ時間になる           |
| 32  | ニョボチップを1億枚もってスタート                           |                                                         |
| 100 | IPボーナス解禁(1000Σでソフトキャップ)                       | ...                                                     |
...
CPを消費して、以下の強力な永続アップグレードを購入できます。
**全てのアップグレードのコストは 1 CP 固定**です。

| アップグレード名         | ID                  | 効果                                                                                                         |
| :----------------------- | :------------------ | :----------------------------------------------------------------------------------------------------------- |
| **根源的な肉**           | `primordialMeat`    | 精肉工場の最終指数(ソフトキャップ後)に `+(Lv * 0.05)` を加算する。                                           |
| **現実改変割引**         | `realityDiscount`   | 工場、アセンション、ギャラクシー(チップ)のコストを割引する。<br>割引率: `1 - (0.97 ^ Lv)` (Lv毎に3%複利割引) |
| **時空を超える無限**     | `ipMultiplier`      | 最終的なIP獲得量が `4 ^ Lv` 倍になる。<br>(break_infinity.jsにより1.79e308を超えて増加可能)                  |
| **ジェネレーター最適化** | `generatorExponent` | GPによる工場強化の最終指数に `+(Lv * 0.02)` を加算する。                                                     |
