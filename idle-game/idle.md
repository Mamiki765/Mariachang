...
初期値はベースが1、グラビティ指数0.3
| アップグレード名 | 効果 | コスト (e12刻み) |
| :--------------- | :--------------------------- | :-------------- |
| ベース値強化 | `+0.1` ずつ | `1e60` IPから |

| アップグレード名   | 効果         | コスト (e21刻み) |
| :----------------- | :----------- | :--------------- |
| グラビティ指数強化 | `+0.05` ずつ | `1e60` IPから    |

- **備考:** 原典のrevoより1000倍速、指数差3.7倍ほどあるとはいえはるかに弱くされている。マリアにもジェネレーター出力に大きなソフトキャップがあるのでここまで怯えた調整はしなくてもいいかも？

#### 9.エタニティ（Σ）（未実装）

ゲーム全体の最終目標。
1.79e+308IP でエンディングとなり、EP（未実装）とΣ（未実装）が追加される。[text](about:blank#blocked)
そして所持ニョボチップとinfinityまでの要素の全てを失う。

#### 9.1 エタニティマイルストーン(未実装)

| Σ数 | 効果                                                   | 備考　                       |
| --- | ------------------------------------------------------ | ---------------------------- |
| 1   | Σに応じたエタニティボーナスの解禁                      | 実質無し                     |
| 2   | ニョボチップがΣ間で維持+最大倍率から減らない           |                              |
| 3   | 最短インフィニティとIC9クリア時間が1ms固定             |                              |
