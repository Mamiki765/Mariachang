...
import { unlockAchievements, unlockHiddenAchievements } from "./achievements.mjs";
...
export function calculateRewards(params) {
    ...
    if (Math.floor(Math.random() * coinConfig.bonus.chance) === 0) {
        const bonus = Math.floor(Math.random() * (coinConfig.bonus.amount.max - coinConfig.bonus.amount.min + 1)) + coinConfig.bonus.amount.min;
        coinAmount += bonus;
        coinBonusMessage = `(ボーナス +${bonus})`;
    }

    // --- 2. ピザ計算 (Mee6詳細版) ---
    const basePizza = Math.floor(Math.random() * (pizzaConfig.baseAmount.max - pizzaConfig.baseAmount.min + 1)) + pizzaConfig.baseAmount.min;
    ...
    if (mee6Level) {
        ...
        // 進捗率 (0.0 ~ 1.0)
        const xpProgress = mee6Level.xpForNextLevel > 0 ? (mee6Level.xpInLevel / mee6Level.xpForNextLevel) : 0;
    ...
    if (member) {
         for (const [roleId, bonusAmount] of Object.entries(pizzaConfig.mee6LevelBonuses)) {
...
export async function executeLoginBonus(client, userId, member, source) {
    ...
    try {
        ...
        const { count } = await supabase.from("booster_status").select("*", { count: "exact", head: true }).eq("user_id", userId);
    ...
    // 計算
    const rewards = calculateRewards({ mee6Level: mee6Info, boosterCount, member });
    ...
    if (source === 'chat') {
        const dmMessage = `雨宿りで発言したため、今日のログインボーナスを自動で受け取りましたにゃ！\n` +
            `- あまやどんぐり: 1個\n` +
        ...
        try {
            await client.users.send(userId, { content: dmMessage }).catch(e => {
                 if (e.code !== 50007) console.error(`[AutoLogin] DM Failed for ${userId}`);
            });
        } catch (e) {}
    }
    ...
    if (idsToCheck.length > 0) await unlockAchievements(client, userId, ...idsToCheck);

    const acornHiddenChecks = [
        { id: 12, condition: updatedPoint.totalacorn >= 50 },
        { id: 13, condition: updatedPoint.totalacorn >= 100 },
      ];
      const idsToCheckHidden = acornHiddenChecks.filter((p) => p.condition).map((p) => p.id);
      if (idsToCheckHidden.length > 0) await unlockHiddenAchievements(client, userId, ...idsToCheckHidden);
