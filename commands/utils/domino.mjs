...
export async function dominoeffect(message, client, id, username, dpname) {
  ...
  if (randomNum === 0) { //ガシャーン！
    ...
    await dominochannel.send({
      flags: [4096],
      content: `# 100　<@${id}>は${currentDomino.totalPlayers}人が並べた${currentDomino.totalCount}枚のドミノを崩してしまいました！\n${currentDomino.attemptNumber}回目の開催は終わり、${escapeDiscordText(username)}の名が刻まれました。`
    });
    ...
    await dominochannel.send({
      flags: [4096],
      content: `Take${dpplayer}:${dpname}が${randomNum}枚ドミノを並べました。現在:${currentDomino.totalCount + randomNum}枚`
    });
    await CurrentDomino.update({
    ...
    if (message.channel.id !== config.dominoch) {
      const replyMessage = await message.reply({
        flags: [4096],
        content: `ドミドミ…Take${currentDomino.totalPlayers + 1}:${currentDomino.totalCount + randomNum}枚`
      });
      setTimeout(() => {
        replyMessage.delete();
      }, 5000);
